...
- Wrists raised above the head score too low to draw or measure, so overhead reach is measured at the elbow. An audit traced much of this to the pipeline rather than the model: the whole camera frame was letterboxed into the model's 192-pixel input with no crop to the person, so a standing body spanned about 144 pixels and a raised wrist only a few. Two of the three fixes named there are now in: the model is shown a square cropped to the person rather than the whole frame, and capture asks for the largest 4:3 mode the camera offers (1280×960 on the development machine, against 640×480 before). **Neither has yet been watched on a live camera with a person in front of it, so whether overhead wrists survive is still unknown.** MoveNet Thunder remains the untried third. Until someone stands in front of it, this is a limitation of this pipeline, not an established fact about the model.
