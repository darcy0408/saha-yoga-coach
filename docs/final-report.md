...
Test status: PASS — 34 tests covering geometry, confidence thresholds, cue limits, instruction-only truthfulness, all authored reference poses on either lead side, deliberate deviations, routine generation/navigation, personalization, persistence, illustration approval, asset checksums, and Java 26 lazy initialization.
...
Known limitations: No verified live camera/ONNX adapter; synthetic demo is not accuracy evidence; six poses are explicitly instruction-only; the six measured poses currently use limited bilateral knee-angle rules; no TTS.
